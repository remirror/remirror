...
function regexSupplant(regex: RegExp | string, map: Record<string, RegExp | string> = {}, flags = '') {
  ...
  if (typeof regex !== 'string') {
    if (regex.global && _flags.indexOf('g') < 0) {
      _flags += 'g';
    }
    if (regex.ignoreCase && _flags.indexOf('i') < 0) {
      _flags += 'i';
    }
    if (regex.multiline && _flags.indexOf('m') < 0) {
      _flags += 'm';
    }
...
function stringSupplant(str: string, map: PlainObject = {}) {
...
const validCCTLD = regexSupplant(
  RegExp(
    '(?:(?:' +
      ...
      'at|as|ar|aq|ao|an|am|al|ai|ag|af|ae|ad|ac|com' +
      ')(?=[^0-9a-zA-Z@]|$))',
  ),
);
