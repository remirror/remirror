...
import { FileSystem, NewlineKind, PackageName } from '@microsoft/node-core-library';
import {
  DocBlock,
  DocCodeSpan,
  DocComment,
...
import { CustomDocNodes } from '../nodes/custom-doc-node-kind';
...
export class MarkdownDocumenter {
  ...
  private _writeHeaderSection(output: DocSection, apiItem: ApiItem): void {
    const metaData = new DocMetaData({
      configuration: this._tsdocConfiguration,
      menu: 'API',
      name: apiItem.displayName,
      route: this._getLinkFilenameForApiItem(apiItem),
    });
    output.appendNode(metaData);
  }
  ...
  private _getFilenameForApiItem(apiItem: ApiItem, withExtension = false): string {
    ...
    return createBaseName(baseName, withExtension);
  }

  private _getLinkFilenameForApiItem(apiItem: ApiItem): string {
    return API_ROOT + this._getFilenameForApiItem(apiItem);
  }
...
const createBaseName = (name: string, withExtension: boolean) =>
  name
    .split('/')
    .map(nameWithoutSlashes =>
      nameWithoutSlashes
        .split('@')
        .map(nameWithoutAt =>
          nameWithoutAt
