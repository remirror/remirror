...
export class CodeMirrorNodeView implements NodeView {
   ...
   * with the inner one, so that any commands executed on the outer editor see an accurate
   * selection.
   */
   ...
   * Because CodeMirror uses a line/column based indexing system, `indexFromPos` is used to
   * convert to an actual character index.
   */
  ...
  /**
   * Selections are also synchronized the other way, from ProseMirror to CodeMirror, using the
   * view's `setSelection` method.
   */
  ...
  /**
   * When the actual content of the code editor is changed, the event handler registered in the
   * node view's constructor calls this method. It'll compare the code block node's current
   * value to the value in the editor, and dispatch a transaction if there is a difference.
   */
  valueChanged(): void {
  ...
  /**
   * A somewhat tricky aspect of nesting editor like this is handling cursor motion across the
   * edges of the inner editor. This node view will have to take care of allowing the user to move
   * the selection out of the code editor. For that purpose, it binds the arrow keys to handlers
   * that check if further motion would ‘escape’ the editor, and if so, return the selection and
   * focus to the outer editor.
   *
   * The keymap also binds keys for undo and redo, which the outer editor will handle, and for
   * ctrl-enter, which, in ProseMirror's base keymap, creates a new paragraph after a code block.
   */
  ...
  /**
   * When an update comes in from the editor, for example because of an undo action, we kind of
   * have to do the inverse of what `valueChanged` did—check for text changes, and if present,
   * propagate them from the outer to the inner editor.
   */
...
/**
 * `computeTextChange` compare two strings and find the minimal change between them.
 *
 * It iterates from the start and end of the strings, until it hits a difference, and returns an
 * object giving the change's start, end, and replacement text, or `null` if there was no change.
 */
