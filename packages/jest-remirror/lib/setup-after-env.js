...
expect.extend({
  toEqualDocument(actual, expected) {
    ...
    expected = typeof expected === 'function' && actual.type && actual.type.schema ? expected(actual.type.schema) : expected;

    if (!(expected instanceof _core.PMNode) || !(actual instanceof _core.PMNode)) {
      return {
        pass: false,
        actual,
        expected,
  ...
  toMatchDocSnapshot(actual) {
    ...
    const removeFirstWord = sentence => sentence ? sentence.split(' ').slice(1).join(' ') : ''; // this change is to ensure we are mentioning test file name only once in snapshot file
    // for integration tests only


    const newTestName = removeFirstWord(currentTestName); // remove ids that may change from the document so snapshots are repeatable

    const transformedDoc = actual; // since the test runner fires off multiple browsers for a single test, map each snapshot to the same one
    ...
    const ret = _jestSnapshot.toMatchSnapshot.call(this, transformedDoc);
...
//# sourceMappingURL=setup-after-env.js.map