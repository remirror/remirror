...
import { useTheme } from './react-providers';
...
interface DropdownProps extends Omit<MenuDropdownItem, 'type' | 'items'>, Partial<MenuStateReturn> {
  menuItems: Array<
...
export const Dropdown = forwardRef<HTMLButtonElement, DropdownProps>((props, ref) => {
  ...
  return (
    <>
      ...
      <ControllableMenu items={menuItems} menuState={menuState} label={label} />
    </>
  );
});
...
interface ControllableMenuProps extends BaseMenuProps {
  /**
 ...
 * `open` prop.
 */
export const ControllableMenu = (props: ControllableMenuProps): JSX.Element => {
