...
By default these checks are run on every push. To prevent these hooks from running by default copy `.config.sample.json` to `.config.json`. This file is read before hooks are run and can cancel checks when configured.
