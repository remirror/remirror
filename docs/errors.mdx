...
collecting the react ref and appending the prosemirror text editor to it. If that's done multiple
times per render then it has problems.
...
const Editor = () => {
  ...
  return <RemirrorProvider manager={manager}><Internal></ RemirrorProvider>
}
```
...
To fix this issue you need to move the getRootProps to a part of the state tree that only updates
when the `RemirrorProvider` updates. There are several ways to do this and one of them is to split
up the `Internal` component into two seperate components.
