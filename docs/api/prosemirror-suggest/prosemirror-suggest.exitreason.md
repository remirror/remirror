...
[Home](./index.md) &gt; [prosemirror-suggest](./prosemirror-suggest.md) &gt; [ExitReason](./prosemirror-suggest.exitreason.md)
...
|  --- | --- | --- |
|  End | <code>&quot;exit-end&quot;</code> | The user has pasted some text with multiple characters or run a command that adds multiple characters.<code>onExit</code> should be called but the previous match should be retested as it's possible that it's been extended. |
...
|  MoveEnd | <code>&quot;move-end&quot;</code> | User has moved out of the suggestion at the end. This can happen via using arrow keys, but can also be via the suggestion no longer matching as the user types, a mouse click or custom command. All that has changed is the cursor position. |
